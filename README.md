...
## 第 8 周交付状态（Redis 缓存 + 限流 + 租户前缀）

| 模块 | 内容 |
|---|---|
| `app/core/redis.py` | Redis 客户端（真实 Redis / fakeredis 降级） |
| `app/core/cache.py` | 缓存封装 —— **key 结构性带租户前缀** |
| `app/core/rate_limit.py` | 滑动窗口限流算法（ZSET + 事务） |
| `app/middleware/rate_limit.py` | 限流中间件（超配额 `42900` / HTTP 429） |
| `notification_service` | 未读数：读时缓存 + 写时失效 |

**无新增 REST 接口**。399 项测试全绿（含 79 项安全测试），覆盖率 95%，
mypy 全绿，CI 绿。

### ★ 本阶段的核心认知：隔离面从「一个」变成「三个」

前 7 周做的是**数据库行级隔离**（`tenant_id` 钩子）。引入 Redis 后多出来
两个**框架覆盖不到**的存储层——它们都不经过 ORM 钩子：

| 隔离面 | 机制 | 漏了的后果 |
|---|---|---|
| 数据库行 | `tenant_id` 钩子 + 仓储守卫 | 看到别家公司数据 |
| **缓存 key** | `cache_key()` 强制前缀 | **读到别人的缓存**（且不报错） |
| **限流 key** | `build_key()` 强制前缀 | **一个租户吃光全平台配额** |

缓存这一面比数据库更危险：数据库至少有 `NOT NULL` 与查询钩子兜底，
而缓存就是个裸 KV——**key 写错了不会抛任何异常**，只会静默串数据。

对策与前两周一致：**做成结构性保证，不靠自觉**。
`cache.py` 每个公开函数的第一个参数都是 `tenant_id`，漏了就少一个必填参数。

### ★ 一处必须说明的取舍：限流**不用 Lua**

方案原本建议「ZSET + Lua 脚本」实现原子滑动窗口，但存在硬冲突：

- 项目的硬要求是**零依赖降级模式**（11.2）：`USE_FAKEREDIS=true` 时无需外部服务
- **fakeredis 不支持 Lua**（实测 `EVAL` 返回 `unknown command 'eval'`），
  要用就得引入 `lupa`（C 扩展）

选择**保住零依赖**：改用 **Redis 事务（MULTI/EXEC）+ ZSET**。
只用基础命令，真实 Redis 与 fakeredis 都支持，且**语义仍是滑动窗口**
（没退化成固定窗口，因此没有「窗口边界双倍突发」的问题）。

代价是「读计数 → 决定写」分两步（事务里读不到中间结果），并发下可能
**略微超限**（几个请求）；但误差方向**偏保守**，不是放行失控。
对「按租户 600/分钟」这种粗粒度限流完全可接受。

### 超限响应的形状

`429` + 业务码 `42900` + 统一信封，响应头回 `X-RateLimit-Limit` /
`X-RateLimit-Remaining`。

★ **限流中间件必须自己构造响应，不能抛异常**：FastAPI 的 `exception_handler`
由 `ExceptionMiddleware` 执行，而它在中间件栈的**内层**——中间件在
`call_next` 之前抛的异常到不了那里，会直接冒泡成 500，业务码与信封全丢。
这正是新增 `responses.fail()` 的原因。

### 热点缓存：未读数

选 `GET /notifications/unread-count`——前端角标的轮询接口，高频读，
后面还跟着一次 COUNT 查询。

一致性用**读时缓存 + 写时失效**（TTL 30s 只是兜底）：所有会改未读数的路径
（`emit` / `mark_one_read` / `mark_all_read`）都主动删缓存，
所以角标更新的延迟是「一次缓存删除」而不是 30 秒。

### 测试（23 项新增）

- **缓存跨租户**：同 key 不同租户互不可见；失效操作也不跨租户
- **限流跨租户**：★ 用「**同一个 `user_id` 分属两个租户**」这个真实场景
  （`User` 是全局表，同账号可加入多家公司）——租户 A 用尽配额，
  租户 B 完全不受影响
- 限流：放行 / 超限 `429`+`42900` / 响应头 / 配额 1 的边界 /
  窗口滑动 / **同一时刻的多次请求各计一次**（member 去重的坑）
- 缓存命中用「**绕过 service 直接改库**」来证明——否则证明不了真的走了缓存
- 无租户上下文的请求（`/health`、登录）不参与租户配额

## 第 9 周交付状态（测试冻结 + 部署配置 + CI）

**无新增接口**。本阶段的产出是**三类「启动后才暴露」问题的检查**，
外加修掉两个真实缺陷。

**415 项测试全绿**（含 79 项安全测试），覆盖率 95%，mypy 全绿，CI 绿。

### ★ 本阶段的核心认知：「本地发现不了」的问题要专门检查

第 9 周的验收标准是 `docker compose up` 一键起，但**本机没有 Docker**。
复盘下来：**「一键起失败」的原因大多不是容器本身**，而是配置里引用了一个
不存在的东西——而这类问题**不需要 Docker 就能查**。

| 检查 | 针对的失败 | 为什么本地看不见 |
|---|---|---|
| `tests/test_migrations.py` | 迁移建不出应用需要的库 | 内存库走 `create_all`，**压根不经过迁移** |
| `tests/test_deployment_config.py` | compose 引用了不存在的模块/文件/变量 | 不用 Docker 就没人解析它 |
| 零依赖端到端冒烟 | 默认值不完整 | 开发机上环境变量往往是设好的 |

### 修掉的两个真实缺陷

**① 迁移链从未被验证过（第 1 周起就存在）**

compose 的 api 命令是 `alembic upgrade head && uvicorn ...`。
迁移若建不出应用需要的库，**服务能正常启动、但第一个碰该表的请求就 500**——
而本地开发完全发现不了，因为内存库走的是 `create_all`。

验证结果：迁移产物与 `create_all` 的库**完全一致**（15 张表、所有列、
所有索引、所有唯一约束）；**用迁移建出的库起真实服务跑完整业务 9/9 通过**。

**② `.env` 缺失导致 `docker compose up` 直接失败**

`.env` 被 `.gitignore` **正确**排除（密钥不该进仓库），
但 compose 三个服务都声明 `env_file: .env` → clone 下来**解析阶段就失败**。
修法：`required: false`（有就用，没有也不报错——关键变量已在 `environment` 里显式给了）。

### 配置检查里最有价值的几条

- **命令里的 Python 模块必须可导入**：`celery -A app.tasks.celery_app`
  指向不存在的模块时，容器以「退出码 1 + 一行 ModuleNotFoundError」结束。
  （第 7 周之前这个模块**真的不存在**，潜伏了 6 周。）
- **环境变量名必须是 settings 的字段**：拼错不报错，只会**静默用默认值**
- **要求 `service_healthy` 的服务必须真的定义了 `healthcheck`**：
  否则 compose 会一直等一个永远不会出现的状态——卡住且无报错
- **api 命令里迁移必须在 uvicorn 之前**

### CI

- 覆盖率门槛 `--cov-fail-under=90`（当前 95%）。
  **只报告不阻断的覆盖率等于没有约束**——它会一路缓慢下滑，
  而每次下滑单独看都「只降了一点点」
- mypy 已是硬失败（第 5 周起）
...
| 环节 | 状态 |
|---|---|
| 迁移产物可用（起真实服务跑通业务） | ✅ 已验证 |
| 零依赖模式（不设任何环境变量） | ✅ 已验证（6/6） |
| compose 配置引用的每个名字存在 | ✅ 已验证（12 项） |
| **真的 `docker compose up` 起容器** | ❌ **本机无 Docker，未验证** |

前三项覆盖了「一键起」失败原因中的绝大多数，但**容器运行时本身
（镜像构建、网络、卷挂载、启动顺序）仍未验证**。如实记录，不当已完成。

下一阶段目标（第 10 周）：性能压测 + 可观测性（Grafana）+ 文档收尾
（PROJECT-PLAN 十二）。
