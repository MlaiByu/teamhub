...
- **git 仓库已初始化**（2026-09-22），默认分支 `main`（CI 只在 `main`/`develop` 触发）。
  初始导入拆成 9 个按「层 / 关注点」分组的提交，便于 review；
  这些分组**不是**可独立运行的增量，实际开发顺序见各提交正文。
- **提交约定（用户 2026-09-22 指定）**：每完成一个操作步骤就提交一次，
  提交信息必须明确说明本次所做修改。沿用 Conventional Commits 前缀
  （`feat` / `fix` / `test` / `chore` / `docs`），正文用 `-` 列表写清改动与**理由**，
  不要只写「更新了 X」。涉及隔离地基或启动路径的改动，正文里点明影响面。
- **`.gitattributes` 强制 LF**。首次初始化时仓库混有 20 个 CRLF 文件
  （集中在 `alembic/`），已归一化。GitHub 远端**尚未配置**（无 remote）。
- 全局 git 身份：`鸣濑白羽 <1260704327@qq.com>`（邮箱与 GitHub 账号 `MlaiByu` 一致，
  提交归属正常）。
